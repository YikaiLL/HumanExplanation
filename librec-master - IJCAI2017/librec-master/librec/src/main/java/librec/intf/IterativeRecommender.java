...
public abstract class IterativeRecommender extends Recommender {
	...
	protected static int numPath;

	//the maximum layer of the hierarchy
	protected static int layer; 

	//the total number of categories in the heirarchy
	protected static int numCates;
	...
	// factorized category-facotr matrix
	...
	// category biases
	protected DenseVector cateBias;
	...
	// save users' information for ReMF
	protected static Map<String, Map<String, String>> userinfo = new HashMap<>(); 
	protected static Map<String,String> continentsinfo = new HashMap<String, String>();

	// save item hierarchy for VHLF
	protected static Map<String, ArrayList<String>> itempath = new HashMap<String, ArrayList<String>>();
	protected static Table<String, String, Double> itemCorrs = HashBasedTable.create();
	protected static Table<String, String, Double> cateCorrs = HashBasedTable.create();
	protected static boolean isUserInfo;
	protected static boolean isHierarchy;
	protected static boolean isIndex;
	protected static DataDAO cateDao;
	protected static DataDAO itemDao;
	...
	public IterativeRecommender(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
		...
		if (resetStatics) {
			...
			isUserInfo = cf.isOn("is.user_info.add");
			if(isUserInfo){
				String infoPath = cf.getPath("dataset.userinfo");
				String continentsPath = cf.getPath("dataset.continentsinfo");
				cateDao = new DataDAO(infoPath);
				try {
					userinfo = cateDao.readUserInfo(infoPath);
					continentsinfo = cateDao.readContinentsInfo(continentsPath);
				} catch (Exception e) {
			...
			isHierarchy = cf.isOn("is.hierarchy.add");
			if (isHierarchy) {
				String infoPath = cf.getPath("dataset.hierarchy");
				cateDao = new DataDAO(infoPath);
				try {
					itempath = cateDao.readHierarchy(infoPath);
					numPath = cateDao.getNumPath();
					layer = cateDao.getLayer();
					numCates = cateDao.getNumCates();
			...
			isIndex = cf.isOn("is.index.add");
			if (isIndex) {
				String itemPath = cf.getPath("dataset.item");
				String catePath = cf.getPath("dataset.category");
				itemDao = new DataDAO(itemPath);
				try {
					itemCorrs = itemDao.readItemCorrs(itemPath);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				cateDao = new DataDAO(catePath);
				try {
					cateCorrs = cateDao.readCateCorrs(catePath);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	protected void initModel() throws Exception {
		...
		C = new DenseMatrix(numCates, numFactors);
