...
public abstract class Recommender implements Runnable {
	...
	public enum Measure {
		...
		D5, D10, Pre5, Pre10, Pre20, Rec5, Rec10, Rec20, MAP, MRR, NDCG, AUC,
		/* execution time */
		TrainTime, TestTime,
		/* loss value */
		Loss
		...
		if (isRankingPred) {
			if (isDiverseUsed)
				evalInfo = String.format("%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f",
						measures.get(Measure.Pre5), measures.get(Measure.Rec5), measures.get(Measure.Pre10),
						measures.get(Measure.Rec10), measures.get(Measure.Pre20), measures.get(Measure.Rec20),measures.get(Measure.AUC), measures.get(Measure.MAP),
				...
				evalInfo = String.format("%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f", measures.get(Measure.Pre5),
						measures.get(Measure.Rec5), measures.get(Measure.Pre10), measures.get(Measure.Rec10), measures.get(Measure.Pre20),measures.get(Measure.Rec20), 
	...
	protected Map<Measure, Double> evalRankings() throws Exception {
		...
		for (int u = 0, um = testMatrix.numRows(); u < um; u++) {
			...
			List<Integer> cutoffs = Arrays.asList(5, 10, 20);
