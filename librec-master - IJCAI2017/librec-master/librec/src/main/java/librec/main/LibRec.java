...
public class LibRec {
	...
	private void print( SparseMatrix data, String type, int fold){
		String catePath = "D:/experiments/Datasets/datasets-IJCAI2017/home/"+type+fold+".txt";
		try {
		...
		for(int u: data.rows()){
			for(int j: data.row(u).getIndex()){
				double rating = data.get(u, j);
				String uid = rateDao.getUserId(u);
				String iid = rateDao.getItemId(j);
				String cateline = uid +" "+iid+" "+rating;
				catelines.add(cateline);
			}
		}
	...
	protected Recommender getRecommender(SparseMatrix[] data, int fold) throws Exception {
		...
		//print(trainMatrix, "training", fold);
		//print(testMatrix, "testing", fold);
		...
		case "mf":
			return new MF(trainMatrix, testMatrix, fold);	
		case "tmf":
			return new TMF(trainMatrix, testMatrix, fold);	
		case "vhlf":
			return new VHLF(trainMatrix, testMatrix, fold);	
		case "sherlock":
			return new Sherlock(trainMatrix, testMatrix, fold);	
		case "cmf":
			return new CMF(trainMatrix, testMatrix, fold);	
		case "svdf":
			return new SVDF(trainMatrix, testMatrix, fold);	
		case "fm":
			return new FM(trainMatrix, testMatrix, fold);	

			/* item ranking */
		case "rl":
			return new RL(trainMatrix, testMatrix, fold);
		case "rld":
			return new RLD(trainMatrix, testMatrix, fold);
		case "pr":
			return new PR(trainMatrix, testMatrix, fold);
		case "rpr":
			return new RPR(trainMatrix, testMatrix, fold);
		case "muller":
			return new Muller(trainMatrix, testMatrix, fold);
		case "mrlr":
			return new MRLR(trainMatrix, testMatrix, fold);
		case "item2vec":
			return new Item2Vec(trainMatrix, testMatrix, fold);
		case "cofactor":
			return new CoFactor(trainMatrix, testMatrix, fold);
		case "mp2vec":
			return new MP2Vec(trainMatrix, testMatrix, fold);
		case "user2vec":
			return new User2Vec(trainMatrix, testMatrix, fold);	
		case "climf":
